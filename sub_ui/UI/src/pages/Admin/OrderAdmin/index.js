...
function OrderAdmin() {
    ...
    const getListOrders = async () => {
        await newRequest.get(`/orders/?page=${currentPage - 1}&size=${pageSize}`, {
            headers: {
                "Authorization": `Bearer ${accesstoken}`
            }
        })
            ...
            .catch(err => {
                notify('Error getting orders: ' + err, 'error')
            })
    }

    const handleUpdateOrder = (orderId) => {
        newRequest.put(`/orders/${orderId}`, {}, {
            headers: {
                "Authorization": `Bearer ${accesstoken}`
            }
        })
            ...
            .catch(err => {
                console.log(err)
                notify('Error updating order: ' + err, 'error')
            })
    }
    ...
    const handleCreateOrder = () => {
        ...
        if (selectedLocker) {
            formData.append('lockerId', selectedLocker)
            flag = true
        }
        if (selectedImage) {
        ...
        if (!flag) {
            ...
            newRequest.post(`/orders/`, formData, {
                headers: {
                    "Authorization": `Bearer ${accesstoken}`
                }
            })
                .then(data => {
                    ...
                    console.log('create order sucessful!: ', data.data)
                })
                .catch(err => {
                    notify('Error creating order: ' + err, 'error')
                })
        }
    }

    const handleGetLockers = () => {
        newRequest.get(`/lockers/list`, {
            headers: {
                "Authorization": `Bearer ${accesstoken}`
            }
        })
    ...
    return (
        <div className="m-10">
            <div className="w-full relative text-standard">
                <button onClick={() => {
                    setOpenAddOrder(state => !state)
                    // handleAddInvoice()
                }}
                    className={`flex items-center justify-center ${openAddOrder ? 'bg-green-500 text-white' :
                ...
                <form className="flex absolute justify-center items-center w-full background-12">
                    <div className={
                        `bg-white px-6 pt-4 rounded-xl pb-4 w-2/4 mt-10 shadow-lg dark:border-strokedark dark:bg-boxdark dark:text-white duration-500 origin-top
                            ${openAddOrder ? 'scale-1 visible h-auto opacity-100' : 'h-0 scale-0 h-0 invisible opacity-0'}`
                    }>
                        <h2 className="text-standard font-semibold leading-7 text-gray-900 mt-2 text-center">Order Box</h2>
                        <div className="">
                            <div className="sm:col-span-3 text-standard w-full my-6">
                                <label htmlFor="roomId" className="block text-gray-900">Select Locker</label>
                                <div className="mt-2 border-none rounded-lg  px-4">
                                    <select onChange={e => setSelectedLocker(e.target.value)} value={selectedLocker} id="roomName" name="roomName" className="text-standard shadow-md border-none bg-transparent py-2 ps-4 shadow-md rounded-md w-full">
                                        {
                                            lockers && lockers.map(lk => (
                                                <option key={lk.id} value={lk.id} className="text-standard">Locker ID: {lk.id}</option>
                                            ))
                                        }
                                    </select>
                                </div>
                        ...
                        <div className="w-full flex justify-center items-center mt-4">
                            <button onClick={handleCreateOrder} type="button" className="px-4 py-3 bg-green-400 text-white rounded-md transition hover:bg-green-500">Submit</button>
                        </div>
                    </div>
                </form >
            </div>

            <div className={`pb-4 relative transition-all duration-500 ${openAddOrder && selectedImage ? 'top-[490px]' : openAddOrder && !selectedImage ? 'top-80' : 'top-0'}`}>
                <div className={`rounded-lg flex-col mb-4 border mt-16 border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1 transition`}>
                    <div className="max-w-full ">
                        <table className="w-full table-auto">
                            ...
                            <tbody>
                                {orders && orders.content.map((order, key) => (
                                    <tr key={key}>
                                        ...
                                        <td className="border-b border-[#eee] py-2 px-2 pl-9 dark:border-strokedark xl:pl-11">
                                            ...
                                            >
                                                {order.image ? <img className="w-68 h-48 object-contain rounded-md" alt="order" src={order.image} /> : '-----'}
                                            </p>
                                        </td>
                                        <td className="border-b border-[#eee] py-2 px-4 dark:border-strokedark">
                                            <p
                                                className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium ${order.status === 'Received'
                                        ...
                                        <td className="border-b border-[#eee] py-2 px-4 dark:border-strokedark">
                                            <p
                                                className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium`}
                                            >
                                                {order.createdAt ? moment(order.createdAt).format("HH:mm:ss DD/MM/YYYY") : '-----'}
                                            </p>
                                        </td>
                                        <td className="border-b border-[#eee] py-2 px-4 dark:border-strokedark">
                                            <p
                                                className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium`}
                                            >
                                                {order.updatedAt ? moment(order.updatedAt).format("HH:mm:ss DD/MM/YYYY") : '-----'}
                                            </p>
                                        </td>
                                        <td className="border-b border-[#eee] py-2 px-4 dark:border-strokedark">
                                            <div className="flex items-center space-x-3.5">
                                                <button onClick={() => { handleUpdateOrder(order.id) }} className={`hover:text-success transition`}>
                                                ...
                                                <button onClick={() => { deleteOrder(order.id) }} className="hover:text-danger transition-colors">
                                                    <svg
